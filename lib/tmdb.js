...
async function fetchWithRetry(url, options = {}, retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.warn(`[TMDB] Attempt ${i + 1} failed (${err.message}), retrying in ${delay}ms...`);
...
export const getMovieDetails = (id) => fetchTMDB(`/movie/${id}`, { append_to_response: "videos,credits,similar" });
